...
#define LOG_OBJC_MAYBE(a,b,c,d,frmt,...) { NSLog(frmt,##__VA_ARGS__); }
...
#define HTTPLogTrace()             LOG_OBJC_MAYBE(HTTP_LOG_ASYNC_TRACE,   httpLogLevel, HTTP_LOG_FLAG_TRACE, \
                                                  HTTP_LOG_CONTEXT, @"%@:%@", [[NSString stringWithUTF8String:__FILE__]lastPathComponent], NSStringFromSelector(_cmd))
...
#define HTTPLogCTrace()               LOG_C_MAYBE(HTTP_LOG_ASYNC_TRACE,   httpLogLevel, HTTP_LOG_FLAG_TRACE, \
                                                  HTTP_LOG_CONTEXT, @"%s[%p]: %@", __FILE__, self, __FUNCTION__)
